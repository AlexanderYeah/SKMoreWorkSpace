...
class LoginHandler(tornado.web.RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        # 获取到提交到后台的用户名和密码 进行验证是否正确
        username = self.get_argument('logname',default=None);
        password = self.get_argument('logpass',default=None);

        # 查询数据库
        db_pwd = user_orm.GetPwdByUsername(username).user_password;
        print(db_pwd)
        #self.write(user_orm.GetPwdByUsername(username));
        # 如果数据库的密码和输入密码一直 就是对的
        if db_pwd == password:
            self.write("ok");
